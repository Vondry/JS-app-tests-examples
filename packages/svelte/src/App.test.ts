...
import { render, screen, within, waitForElementToBeRemoved } from '@testing-library/svelte';
...
describe('App', () => {
    ...
    test("todo can be completed by clicking on it's description", async () => {
        ...
        expect(await within(component).findByText('Press to complete')).toBeInTheDocument();
        ...
        expect(await within(component).findByText('Press to uncomplete')).toBeInTheDocument();
    });
    ...
    test('todo can be deleted', async () => {
        ...
        // Wait until element is not removed
        await waitForElementToBeRemoved(screen.queryByText(firstTodo.description));
